...
export class BookingComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.bookingService.getBookingDays().subscribe(response => {
      this.bookingsDays = response.reduce<
        Partial<Record<string, BookingDay[]>>
      >((acc, day) => {
        const dayOfTheWeekName =
          Object.values(DaysOfWeek)[day.day_of_week].toString();
  ...
  async onSubmit(form: FormGroup): Promise<void> {
    ...
    if (form.valid && this.selectedDate && this.selectedHour) {
      ...
      const date = new Date(
        `${this.selectedDate.formattedDate} ${this.selectedHour.hour}`
      );
